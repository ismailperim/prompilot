...
from tests.conftest import TIMESERIES_SPEC, D, json_response, load_fixture, mock_prometheus


def test_empty_dashboard(client: TestClient) -> None:
    body = client.get(f"{D}").json()
...
def test_panel_crud_flow(client: TestClient) -> None:
    created = client.post(f"{D}/panels", json={"spec": TIMESERIES_SPEC})
    ...
    patched = client.patch(f"{D}/panels/{panel_id}", json={"options": {"draw": "bars"}})
    assert patched.status_code == 200
    assert patched.json()["spec"]["options"]["draw"] == "bars"

    replaced = client.put(f"{D}/panels/{panel_id}", json={**TIMESERIES_SPEC, "title": "Replaced"})
    ...
    deleted = client.delete(f"{D}/panels/{panel_id}")
    assert deleted.status_code == 204
    assert client.delete(f"{D}/panels/{panel_id}").status_code == 404
    assert client.get(f"{D}").json()["panels"] == []


def test_invalid_spec_returns_422_with_messages(client: TestClient) -> None:
    response = client.post(f"{D}/panels", json={"spec": {**TIMESERIES_SPEC, "type": "pie"}})
...
def test_dashboard_settings(client: TestClient) -> None:
    response = client.patch(
        f"{D}", json={"title": "Prod", "timeRange": {"from": "now-6h", "to": "now"}}
    )
    assert response.status_code == 200
    assert response.json()["title"] == "Prod"
    assert response.json()["timeRange"]["from"] == "now-6h"

    response = client.patch(f"{D}", json={"refresh": "10x"})
    assert response.status_code == 422
...
class TestData:
    ...
    def test_batch_data_for_all_panels(self, client: TestClient) -> None:
        ...
        a = client.post(f"{D}/panels", json={"spec": TIMESERIES_SPEC}).json()["spec"]["id"]
        b = client.post(
            f"{D}/panels",
            json={
                "spec": {
        ...
        response = client.post(f"{D}/panels/data", json={})
    ...
    def test_batch_data_with_ids_and_time_range(self, client: TestClient) -> None:
        ...
        self._prometheus(client, calls)
        a = client.post(f"{D}/panels", json={"spec": TIMESERIES_SPEC}).json()["spec"]["id"]
        client.post(f"{D}/panels", json={"spec": TIMESERIES_SPEC})

        response = client.post(
            f"{D}/panels/data", json={"ids": [a], "timeRange": {"from": "now-6h", "to": "now"}}
        )
        body = response.json()
    ...
    def test_query_error_is_reported_per_panel(self, client: TestClient) -> None:
        ...
        mock_prometheus(client, handler)
        good = client.post(f"{D}/panels", json={"spec": TIMESERIES_SPEC}).json()["spec"]["id"]
        bad = client.post(
            f"{D}/panels",
            json={"spec": {**TIMESERIES_SPEC, "queries": [{"refId": "A", "expr": "rate(bad"}]}},
        ).json()["spec"]["id"]

        body = client.post(f"{D}/panels/data", json={}).json()
    ...
    def test_single_panel_data_with_time_from_override(self, client: TestClient) -> None:
        ...
        panel_id = client.post(
            f"{D}/panels", json={"spec": {**TIMESERIES_SPEC, "timeFrom": "24h"}}
        ).json()["spec"]["id"]

        response = client.get(f"{D}/panels/{panel_id}/data")
        assert response.status_code == 200
    ...
    def test_single_panel_data_404(self, client: TestClient) -> None:
        assert client.get(f"{D}/panels/ghost/data").status_code == 404

    def test_bad_time_range_is_400(self, client: TestClient) -> None:
        response = client.post(
            f"{D}/panels/data", json={"timeRange": {"from": "yesterday", "to": "now"}}
        )
    ...
    def test_unreachable_prometheus_is_per_panel_error(self, client: TestClient) -> None:
        ...
        mock_prometheus(client, handler)
        panel_id = client.post(f"{D}/panels", json={"spec": TIMESERIES_SPEC}).json()["spec"]["id"]
        body = client.post(f"{D}/panels/data", json={}).json()
