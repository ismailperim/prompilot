...
from tests.conftest import canned, json_response, load_fixture, mock_prometheus
...
def test_chat_streams_agent_events(llm_client: TestClient) -> None:
    ...
    with llm_client.stream(
        "POST", "/api/chat", json={"message": "add up", "history": []}
    ) as response:
    ...
    assert len(llm_client.get("/api/dashboard").json()["panels"]) == 1


def test_chat_validates_request(llm_client: TestClient) -> None:
    assert llm_client.post("/api/chat", json={"message": ""}).status_code == 422
    assert (
        llm_client.post(
            "/api/chat", json={"message": "x", "history": [{"role": "tool", "content": "y"}]}
        ).status_code
        == 422
    )
