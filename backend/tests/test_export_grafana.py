...
from tests.conftest import TIMESERIES_SPEC, D
...
def test_export_endpoint_downloads_json(client: TestClient) -> None:
    client.patch(f"{D}", json={"title": "My Board"})
    client.post(f"{D}/panels", json={"spec": TIMESERIES_SPEC})

    response = client.get(f"{D}/export/grafana")
    ...
    inline = client.get(f"{D}/export/grafana", params={"download": "false"})
