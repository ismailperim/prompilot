...
import { SERIES_COLORS } from './timeseriesOption'
...
export function Stat({ spec, frames, timeRange }: PanelRendererProps) {
  ...
  return (
    <div className={`stat stat--${shown.length > 1 ? 'grid' : 'single'}`}>
      {shown.map((s, i) => (
        <StatTile key={s.name + i} series={s} opts={opts} spec={spec} timeRange={timeRange} showName={shown.length > 1} />
      ))}
    </div>
  )
}
...
function StatTile({ series, opts, spec, timeRange, showName }: TileProps) {
  ...
  const color = neutral ? 'var(--fg)' : thresholdColor(series.value, opts.thresholds)
  ...
  return (
    <div className="stat__tile" style={{ background: filled ? color : undefined }}>
      {opts.graph && series.points.length > 1 && (
        <Sparkline points={series.points} color={filled ? 'rgba(11,14,20,0.35)' : neutral ? '#f0883e' : color} timeRange={timeRange} />
      )}
      <div className="stat__content">
        {showName && (
          <span className="stat__name mono" style={{ color: filled ? '#0b0e14' : undefined }}>
            {series.name}
          </span>
        )}
        <span className="stat__value" style={{ color: filled ? '#0b0e14' : color }}>
